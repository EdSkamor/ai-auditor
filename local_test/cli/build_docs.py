...
class BuildDocsCLI(BaseCLI):
    ...
    def _setup_docs_args(self) -> None:
        ...
        self.parser.add_argument(
            "--html", action="store_true", help="Build HTML documentation"
        )
        self.parser.add_argument(
            "--pdf", action="store_true", help="Build PDF documentation"
        )
        self.parser.add_argument(
            "--latex", action="store_true", help="Build LaTeX documentation"
        )
        self.parser.add_argument(
            "--all", action="store_true", help="Build all documentation formats"
        )

        # Quality checks
        self.parser.add_argument(
            "--check-docstrings", action="store_true", help="Check docstring coverage"
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "--warn-as-error", action="store_true", help="Treat warnings as errors"
        )
        ...
        self.parser.add_argument(
            "--nitpicky", action="store_true", help="Run in nitpicky mode (more strict)"
        )
    ...
    def _validate_args(self, args: argparse.Namespace) -> None:
        ...
        if not (args.html or args.pdf or args.latex or args.all):
            raise CLIError(
                "At least one build format must be specified", ExitCode.INVALID_ARGS
            )
    ...
    def _run_impl(self, args: argparse.Namespace, config: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if not self._check_sphinx_installed():
                raise CLIError(
                    "Sphinx is not installed. Install with: pip install sphinx",
            ...
            if args.pdf or args.all:
                ...
                if result != 0:
                    ...
                    if latex_dir.exists():
                        ...
                        try:
                            subprocess.run(
                                ["make", "all-pdf"], cwd=latex_dir, check=True
                            )
