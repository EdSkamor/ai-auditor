...
class DataProcessor:
    ...
    def extract_prompts_from_header(self, df: pd.DataFrame, header_row: int) -> List[str]:
    ...
    def parse_amount_series(self, series: pd.Series) -> pd.Series:
        ...
        # Convert to string and remove non-numeric characters except separators
        s = series.astype(str).str.replace(r'[^\d,.\-]', '', regex=True)
        
        # Handle thousands separators (remove dots that are followed by more dots)
    ...
    def find_column_by_candidates(self, candidates: List[str], columns: List[str]) -> Optional[str]:
...
class FileIngester:
    ...
    def read_excel_file(self, file_bytes: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            df_sniff = pd.read_excel(io.BytesIO(file_bytes), header=None, dtype=str, engine="openpyxl")
            ...
            df = pd.read_excel(io.BytesIO(file_bytes), header=header_row, dtype=str, engine="openpyxl")
    ...
    def read_csv_file(self, file_bytes: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            # Read without header first to find the best header row
            df_sniff = pd.read_csv(io.StringIO(text_content), header=None, dtype=str, sep=None, engine="python")
            header_row = self.processor.find_header_row(df_sniff)
            prompts = self.processor.extract_prompts_from_header(df_sniff, header_row)
            
            # Read with proper header
            df = pd.read_csv(io.StringIO(text_content), header=header_row, dtype=str, sep=None, engine="python")
            df.columns = self.processor.flatten_multiindex_columns(df.columns)
    ...
    def read_table(self, file_bytes: bytes, filename: str) -> Dict[str, Any]:
        ...
        if file_path.suffix in {'.xlsx', '.xls'}:
            ...
            raise FileProcessingError(f"Unsupported file format: {filename}", filename, file_path.suffix)
...
class DataAnalyzer:
    ...
    def analyze_table(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        date_col = self.processor.find_column_by_candidates(
            [col for col in columns if col.startswith("data")], 
            columns
        )
        
        amount_candidates = [
            "wartosc_netto_dokumentu", "wartosc_brutto_dokumentu",
            "kwota_netto", "kwota_brutto", "kwota", 
            "wartosc_netto", "wartosc_brutto"
        ]
        amount_col = self.processor.find_column_by_candidates(amount_candidates, columns)
        
        counterparty_candidates = [
            "kontrahent", "nabywca", "dostawca", 
            "klient", "sprzedawca", "odbiorca"
        ]
        counterparty_col = self.processor.find_column_by_candidates(counterparty_candidates, columns)
        ...
        if counterparty_col is not None and amount_col is not None:
            counterparty_data = self._analyze_counterparties(df, counterparty_col, amount_col)
            result.update(counterparty_data)
        
        return result
    
    def _analyze_monthly_trends(self, df: pd.DataFrame, date_col: str, amount_col: str) -> Dict[str, Any]:
        """Analyze monthly trends in the data."""
        try:
            temp_df = df[[date_col, amount_col]].copy()
            temp_df[amount_col] = self.processor.parse_amount_series(temp_df[amount_col])
            ...
            result = {
                "monthly": [[d.strftime("%Y-%m"), float(v)] for d, v in monthly_series.items()]
            }
            ...
            if len(monthly_series) >= 2:
                ...
                result["mom_pct"] = float((latest - previous) / (previous + 1e-9) * 100.0)
    ...
    def _analyze_counterparties(self, df: pd.DataFrame, counterparty_col: str, amount_col: str) -> Dict[str, Any]:
        ...
        try:
            ...
            counterparty_totals = temp_df.groupby(counterparty_col, dropna=False)[amount_col].sum()
            top_counterparties = counterparty_totals.sort_values(ascending=False).head(10)
            
            return {
                "top_counterparties": [[str(k), float(v)] for k, v in top_counterparties.items()],
