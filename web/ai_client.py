...
from .config import AI_API_BASE, AI_HEALTH, AI_TIMEOUT
...
class AIClient:
    ...
    def __init__(self, base: str = AI_API_BASE, timeout: float = AI_TIMEOUT):
        ...
        self.session.headers.update(
            {"Content-Type": "application/json", "User-Agent": "AI-Auditor-Web/1.0"}
        )

    def health(self) -> Dict[str, Any]:
        """Check AI API health status."""
        try:
            response = self.session.get(f"{self.base}{AI_HEALTH}", timeout=self.timeout)
    ...
    def analyze(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                f"{self.base}/analyze", json=payload, timeout=self.timeout
            )
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
    ...
    def wait_until_ready(self, retries: int = 10, delay: float = 1.0) -> bool:
        ...
        for attempt in range(retries):
            try:
                ...
                if health_data.get("ready") or health_data.get("status") in {
                    "ok",
                    "healthy",
                    "ready",
                }:
