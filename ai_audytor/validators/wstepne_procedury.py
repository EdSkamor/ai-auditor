...
class FormulaEvaluator:
                ...
                if start_row is not None and end_row is not None:
                    for row in range(
                        ...
                        for col in range(
                            ...
                            sheet_name = (
                                list(self.sheets.keys())[0] if self.sheets else ""
                            )
    ...
    def _evaluate_if(self, args: List[str]) -> Any:
        ...
        try:
            ...
            if eval(condition):
                return (
                    true_value.strip('"') if isinstance(true_value, str) else true_value
                )
            else:
                return (
                    false_value.strip('"')
...
def eval_formulas(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for sheet_name, sheet_data in data.get("sheets", {}).items():
        ...
        evaluated_sheet = {"name": sheet_name, "formulas": {}, "evaluated_data": []}
            ...
            for col_idx, value in row_data.items():
                if isinstance(value, str) and value.startswith("="):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error evaluating formula {value} in {sheet_name}: {e}"
                        )
    ...
    logger.info(
        f"Evaluated {results['successful_evaluations']}/{results['total_formulas']} formulas"
    )
    return results


def to_report(
    data: Dict[str, Any], results: Dict[str, Any]
    ...
    for sheet_name, sheet_results in results.get("evaluated_sheets", {}).items():
        ...
        report["sheets_summary"][sheet_name] = {
            ...
            "cols": (
                len(sheet_results.get("evaluated_data", [0]))
                if sheet_results.get("evaluated_data")
                else 0
            ),
        }
...
def process_wstepne_procedury(json_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "data": data, "results": results, "report": report}
    except Exception as e:
