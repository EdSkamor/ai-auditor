...
from core.ocr_etl import ETLProcessor, DocumentType, OCREngine, DocumentClassifier, FieldExtractor
...
def test_ocr_etl():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        test_texts = [
            ...
            ("UMOWA\nNr umowy: UM-001/2024\nZawarta w dniu: 15.01.2024", "umowa_test.pdf"),
            ("WYCIĄG BANKOWY\nRachunek: 1234567890\nOkres: 01.01.2024 - 31.01.2024", "wyciag_test.pdf"),
            ("DEKLARACJA VAT\nOkres: 01.2024\nPodatek: 1000,00 zł", "deklaracja_test.pdf")
        ]
        
        for text, filename in test_texts:
            classification = classifier.classify(text, filename)
            print(f"   • {filename}: {classification.document_type.value} (confidence: {classification.confidence:.2f})")
        ...
        fields = field_extractor.extract_fields(invoice_text, DocumentType.INVOICE, mock_ocr_result)
        ...
        if stats:
            ...
            print(f"   • Average classification confidence: {stats.get('average_classification_confidence', 0):.3f}")
            print(f"   • Average OCR confidence: {stats.get('average_ocr_confidence', 0):.3f}")
        ...
        try:
            ...
            for result in results:
                print(f"   • {result.document_id}: {result.document_type.value} ({result.classification_confidence:.2f})")
