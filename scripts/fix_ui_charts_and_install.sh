...
echo "[info] Installing matplotlib into .venv…"
. .venv/bin/activate
pip install --upgrade pip wheel setuptools >/dev/null
pip install "matplotlib>=3.8,<3.10" >/dev/null
echo "[ok] matplotlib installed."

echo "[info] Writing app/ui_charts.py…"
cat > app/ui_charts.py <<'PY'
...
def st_reset_filters_button(keys_to_clear: Iterable[str], label: str = "🔄 Reset filtrów") -> bool:
    ...
    return False
PY
echo "[ok] app/ui_charts.py written."
