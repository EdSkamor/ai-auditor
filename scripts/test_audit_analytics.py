...
from core.audit_analytics import (
    AuditAnalytics, RiskAnalyzer, JournalEntryAnalyzer, SamplingEngine,
    RiskLevel, RiskCategory, SamplingMethod, JournalEntry, RiskFactor
)
...
def test_audit_analytics():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for factor in risk_assessment.risk_factors:
            ...
            print(f"     - {factor.name}: {factor.level.value} (score: {risk_score:.1f})")
        ...
        weekend_entry = JournalEntry(
            id="JE_WEEKEND",
            date=datetime.now() - timedelta(days=datetime.now().weekday() - 5),  # Saturday
            account_code="1001",
        ...
        for factor in risk_table['risk_factors']:
            print(f"     - {factor['name']}: {factor['level']} (score: {factor['risk_score']:.1f})")
        ...
        print(f"   • Custom risk assessment: {custom_assessment.overall_risk_level.value}")
        ...
        test_population = [{'id': f'item_{i}', 'amount': random.uniform(100, 1000)} for i in range(100)]
        ...
        try:
            ...
            invalid_result = analytics.perform_sampling(test_population, "INVALID_METHOD")
