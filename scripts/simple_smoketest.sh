...
say "Wait for model to be ready (max 60s)"
python - <<'PY'
import time,requests,sys
for i in range(60):
    try:
        r=requests.get("http://127.0.0.1:8000/ready",timeout=2)
        if r.ok:
            data = r.json()
            if data.get("model_ready", False):
                print("Model ready!", flush=True)
                sys.exit(0)
        print("Model not ready yet...", flush=True)
    except Exception as e:
        print("wait", type(e).__name__, flush=True)
    time.sleep(1)
print("TIMEOUT waiting for model", flush=True); sys.exit(1)
PY

say "Curl smoke - analyze endpoint"
set -x
...
if [ -f tests/test_e2e.py ]; then
  ...
  pytest -q -x --maxfail=1 --timeout=90 tests/test_e2e.py
else
