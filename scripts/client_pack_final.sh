...
EXPOSE 8501
HEALTHCHECK --interval=30s --timeout=5s --start-period=60s --retries=3 CMD ["curl","-fsS","http://localhost:8501/healthz"]
ENTRYPOINT ["streamlit","run","app/Home.py","--server.port=8501","--server.address=0.0.0.0"]
DFG
...
# --- QUICKSTART (1 plik referencyjny) ---
...
printf "[DONE] client_pack_final completed.\n"
