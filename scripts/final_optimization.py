...
class SystemOptimizer:
        ...
        if data_dir.exists():
            for db_file in data_dir.rglob("*.db"):
                try:
                    ...
                    db_files.append(
                        {"path": str(db_file), "size_mb": round(size / (1024**2), 2)}
                    )
                except (OSError, IOError):
                    continue
    ...
    def run_performance_tests(self) -> Dict[str, Any]:
        ...
        tests["cpu_performance"] = {
            ...
            "status": (
                "good" if cpu_time < 1.0 else "warning" if cpu_time < 2.0 else "slow"
            ),
        }
        ...
        tests["memory_performance"] = {
            ...
            "status": (
                "good"
                if memory_time < 0.1
                else "warning" if memory_time < 0.5 else "slow"
            ),
        }
        ...
        tests["io_performance"] = {
            ...
            "status": (
                "good" if io_time < 0.1 else "warning" if io_time < 0.5 else "slow"
            ),
        }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if "system_resources" in self.optimization_results:
            ...
            if resources["cpu"]["status"] == "critical":
                recommendations.append(
                    "⚠️ CPU usage is critical - consider reducing workload"
                )
            elif resources["cpu"]["status"] == "warning":
                recommendations.append(
                    "⚠️ CPU usage is high - monitor system performance"
                )

            if resources["memory"]["status"] == "critical":
                recommendations.append(
                    "⚠️ Memory usage is critical - consider adding RAM"
                )
            ...
            if resources["disk"]["status"] == "critical":
                recommendations.append(
                    "⚠️ Disk space is critical - free up space immediately"
                )
        ...
        if "network_optimization" in self.optimization_results:
            ...
            if net_opt["recommendation"] == "review":
                recommendations.append(
                    "🌐 Review network connections - high number detected"
                )
        ...
        if "performance_tests" in self.optimization_results:
            ...
            for test_name, test_result in perf_tests.items():
                if test_result["status"] == "slow":
                    recommendations.append(
                        f"⚡ {test_name} is slow - consider hardware upgrade"
                    )
