...
class BuildDocsCLI(BaseCLI):
    ...
    def __init__(self):
        ...
        self.parser.add_argument(
            "--install-deps", action="store_true", help="Install Sphinx dependencies"
        )
    ...
    def _run_impl(self, args):
        ...
        try:
            ...
            if args.watch:
                ...
                result = subprocess.run(["make", "-C", str(docs_dir), "watch"])
            else:
                # Single build
                result = subprocess.run(["make", "-C", str(docs_dir), args.format])
            ...
            if args.serve and args.format == "html":
                ...
                subprocess.run(["make", "-C", str(docs_dir), "serve"])
