...
def test_ai_assistant_initialization():
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                len(assistant.rag_system.document_store.documents) > 0
            )  # Should have default docs

            print("✅ AI Assistant initialization working")
            print(f"   Store directory: {assistant.store_dir}")
            print(
                f"   Documents loaded: {len(assistant.rag_system.document_store.documents)}"
            )
            return True
...
def test_document_addition():
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            retrieved_doc = assistant.rag_system.document_store.get_document(
                "custom_doc"
            )
            assert retrieved_doc is not None
