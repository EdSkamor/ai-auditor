...
def test_frontend_logic():
    ...
    export_names = [
        'Lista PBC', 'Status PBC', 'Timeline PBC',
        'Working Papers', 'Łańcuch dowodowy', 'Statystyki WP',
        'Raport końcowy', 'Executive Summary', 'Compliance Report'
    ]
    
    for i, (export_type, export_name) in enumerate(zip(
        ['PBC'] * 3 + ['WP'] * 3 + ['Report'] * 3,
        export_names
    )):
        export = {
...
def test_ui_components():
    ...
    status_classes = ['status-running', 'status-completed', 'status-failed', 'status-pending']
    ...
    for shortcut, action in shortcuts.items():
        ...
        assert action in ['Run', 'Findings', 'Exports', 'Upload', 'Refresh', 'Dark mode']
...
def test_data_structures():
    ...
    required_job_fields = ['id', 'name', 'status', 'progress', 'file_count', 'created_at', 'files']
    ...
    required_finding_fields = ['id', 'title', 'category', 'severity', 'description', 'date', 'file']
