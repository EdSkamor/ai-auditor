...
class POPMatcher:
    ...
    def __init__(
        ...
        self.column_mappings = {
            ...
            "date": ["data", "date", "data_dokumentu", "issue_date", "document_date"],
            "amount_net": [
                "netto",
                "net",
                "total_net",
    ...
    def _normalize_vendor_name(self, vendor: str) -> str:
        ...
        normalized = re.sub(
            r"\s+(SP\.?\s*Z\s*O\.?O\.?|LTD\.?|LLC\.?|INC\.?)$", "", normalized
        )
    ...
    def _calculate_tie_breaker_score(
        self, seller_similarity: float, filename_hit: bool
    ) -> float:
        """Calculate tie-breaker score."""
        fname_score = 1.0 if filename_hit else 0.0
        seller_score = (
            seller_similarity if seller_similarity >= self.tiebreak_min_seller else 0.0
        )
    ...
    def _match_by_number(
        ...
        for idx, row in pop_df.iterrows():
            pop_number = self._normalize_invoice_number(
                str(row[column_map["invoice_number"]])
            )
    ...
    def _match_by_date_amount(
        ...
        for idx, row in pop_df.iterrows():
            try:
                ...
                amount_diff = abs(pdf_amount - pop_amount) / max(
                    pdf_amount, pop_amount, 1
                )
    ...
    def match_invoice(
        self, invoice_data: Dict[str, Any], pop_df: pd.DataFrame, filename: str = ""
    ) -> MatchResult:
        ...
        try:
            ...
            if number_matches:
                ...
                if len(number_matches) > 1:
                    ...
                    return MatchResult(
                        ...
                        criteria=(
                            MatchCriteria.NUMBER_FNAME
                            if best_match[2]
                            else MatchCriteria.NUMBER_SELLER
                        ),
            ...
            date_amount_matches = self._match_by_date_amount(
                invoice_data, pop_df, column_map
            )
    ...
    def _apply_tie_breaker(
        ...
        for idx, confidence in matches:
            ...
            if "vendor" in column_map:
                ...
                seller_similarity = self._calculate_seller_similarity(
                    pop_vendor, pdf_seller
                )
            ...
            if filename_invoice_id and "invoice_number" in column_map:
                pop_number = self._normalize_invoice_number(
                    str(pop_row[column_map["invoice_number"]])
                )
                filename_hit = pop_number == filename_invoice_id

            # Calculate tie-breaker score
            tie_breaker_score = self._calculate_tie_breaker_score(
                seller_similarity, filename_hit
            )
    ...
    def _generate_comparison_flags(
        self,
        invoice_data: Dict[str, Any],
        pop_row: pd.Series,
        column_map: Dict[str, str],
        ...
        if "invoice_number" in column_map:
            pdf_number = self._normalize_invoice_number(
        ...
        if "date" in column_map:
            ...
            if pdf_date:
                try:
                    pop_date = pd.to_datetime(
                        str(pop_row[column_map["date"]]), errors="coerce", dayfirst=True
                    )
            ...
            if pdf_amount is not None:
                try:
                    ...
                    if not pd.isna(pop_amount):
                        ...
                        flags["amount_match"] = (
                            "TAK" if amount_diff <= self.amount_tolerance else "NIE"
                        )
                    else:
        ...
        if "vendor" in column_map:
            ...
            flags["seller_match"] = (
                "TAK" if seller_similarity >= self.tiebreak_min_seller else "NIE"
            )
...
def match_invoice(
    invoice_data: Dict[str, Any], pop_df: pd.DataFrame, filename: str = ""
) -> MatchResult:
