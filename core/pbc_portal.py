...
from .exceptions import AuditorException, ValidationError, FileProcessingError
...
class PBCPortal:
    ...
    def _load_data(self):
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(self.assignments)} assignments and {len(self.working_papers)} working papers")
    ...
    def _save_data(self):
        ...
        try:
            # Save assignments
            assignments_file = self.data_dir / 'assignments.json'
            assignments_data = [asdict(assignment) for assignment in self.assignments.values()]
    ...
    def create_assignment(self, 
        ...
        try:
            ...
            if assignment_type in self.pbc_templates:
                assignment.pbc_items = [PBCItem(**asdict(item)) for item in self.pbc_templates[assignment_type]]
    ...
    def list_assignments(self, 
                        ...
                        auditor: str = None,
                        client: str = None) -> List[Assignment]:
        ...
        if client:
            assignments = [a for a in assignments if client.lower() in a.client_name.lower()]
    ...
    def update_pbc_item_status(self, assignment_id: str, item_id: str, status: TaskStatus, comment: str = None) -> bool:
        ...
        for item in assignment.pbc_items:
            if item.id == item_id:
                ...
                if comment:
                    item.comments.append(f"{datetime.now().strftime('%Y-%m-%d %H:%M')}: {comment}")
    ...
    def add_working_paper(self, 
                         assignment_id: str,
                         title: str,
                         content: str,
                         file_path: str = None,
                         created_by: str = None) -> WorkingPaper:
    ...
    def export_working_papers(self, assignment_id: str, output_dir: Path = None) -> Path:
        ...
        try:
            ...
            wps = [wp for wp in self.working_papers.values() if wp.assignment_id == assignment_id]
            
            if not wps:
                raise ValidationError(f"No working papers found for assignment {assignment_id}")
            
            # Create ZIP file
            zip_path = output_dir / f"working_papers_{assignment_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
            ...
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                manifest = {
                    ...
                    'files': [f"{wp.id}_{wp.title.replace(' ', '_')}.txt" for wp in wps]
                }
    ...
    def get_assignment_timeline(self, assignment_id: str) -> List[Dict[str, Any]]:
        ...
        timeline.append({
            ...
            'status': 'completed' if assignment.status != AssignmentStatus.DRAFT else 'pending',
        ...
        for item in assignment.pbc_items:
            if item.deadline:
                timeline.append({
                    ...
                    'status': 'completed' if item.status == TaskStatus.COMPLETED else 'pending',
        ...
        timeline.append({
            ...
            'status': 'completed' if assignment.status == AssignmentStatus.COMPLETED else 'pending',
    ...
    def get_assignment_statistics(self, assignment_id: str) -> Dict[str, Any]:
        ...
        completed_items = len([item for item in assignment.pbc_items if item.status == TaskStatus.COMPLETED])
        pending_items = len([item for item in assignment.pbc_items if item.status == TaskStatus.PENDING])
        in_progress_items = len([item for item in assignment.pbc_items if item.status == TaskStatus.IN_PROGRESS])
        ...
        return {
            ...
            'working_papers_count': len([wp for wp in self.working_papers.values() if wp.assignment_id == assignment_id])
        }
