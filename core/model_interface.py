...
try:
    ...
    class MockTorch:
        class float16: pass
        def ones_like(self, *args, **kwargs): return None
...
class HuggingFaceModel(ModelInterface):
    ...
    def __init__(self, 
        ...
        self.adapter_dir = adapter_dir or (Path(__file__).parent.parent / "outputs" / "lora-auditor")
    ...
    def _load_model(self) -> None:
        ...
        if not HAS_TORCH:
            raise ModelLoadError("PyTorch and transformers not available. Install with: pip install torch transformers", self.model_name)
        ...
        try:
            ...
            raise ModelLoadError(f"Failed to load model {self.model_name}: {str(e)}", self.model_name)
...
class LlamaCppModel(ModelInterface):
    ...
    def __init__(self, 
        ...
        self.llama_bin = llama_bin or (Path(__file__).parent.parent / "third_party" / "llama.cpp" / "build" / "bin" / "llama-cli")
        self.model_path = model_path or (Path.home() / "models" / "llama3" / "meta-llama-3-8b-instruct.Q4_K_M.gguf")
        self.ngl = ngl
