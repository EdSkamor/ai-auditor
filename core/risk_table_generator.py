...
class RiskTableGenerator:
    ...
    def create_risk_assessment(
        ...
        high_risks = len(
            [
                r
                for r in risk_items
                if self.get_risk_level(
                    self.calculate_risk_score(r.impact, r.probability)
                )
                == "WYSOKIE"
            ]
        )
        ...
        if total_risks > 0:
            total_score = sum(
                self.calculate_risk_score(r.impact, r.probability) for r in risk_items
            )
    ...
    def generate_excel_workbook(
        self, assessment: RiskAssessment, output_path: Path
    ) -> Path:
    ...
    def _create_summary_sheet(
        ...
        worksheet.write(
            f"B{row}",
            f"{assessment.period_start.strftime('%d.%m.%Y')} - {assessment.period_end.strftime('%d.%m.%Y')}",
            formats["border"],
        )
        row += 2
        ...
        worksheet.merge_range(
            f"A{row}:H{row}", "PODSUMOWANIE RYZYK", formats["subheader"]
        )
        row += 1
        ...
        for category_name, category in self.risk_categories.items():
            category_risks = [
                r for r in assessment.risk_items if r.category == category_name
            ]
            if category_risks:
                avg_score = sum(
                    self.calculate_risk_score(r.impact, r.probability)
    ...
    def _create_risk_matrix_sheet(
        ...
        for risk in assessment.risk_items:
            ...
            worksheet.write(
                f"B{row}", self.risk_categories[risk.category].name, formats["border"]
            )
    ...
    def _create_detailed_risks_sheet(
        self,
        workbook: xlsxwriter.Workbook,
        assessment: RiskAssessment,
        formats: Dict[str, Any],
        ...
        for risk in assessment.risk_items:
            ...
            worksheet.write(
                f"C{row}", self.risk_categories[risk.category].name, formats["border"]
            )
    ...
    def _create_mitigation_plan_sheet(
        self,
        workbook: xlsxwriter.Workbook,
        assessment: RiskAssessment,
        formats: Dict[str, Any],
    ...
    def _create_assumptions_sheet(
        ...
        assumptions = [
            (
                "Metodologia oceny:",
                "Ocena ryzyka oparta na macierzy wpływu i prawdopodobieństwa",
            ),
            ...
            (
                "Data generowania:",
                assessment.assessment_date.strftime("%d.%m.%Y %H:%M"),
            ),
