...
class PaymentValidator:
    ...
    def _validate_iban(self, payment: Payment) -> Dict[str, Any]:
        ...
        if not self._is_valid_iban(payment.receiver_account):
            result["errors"].append(
                f"Invalid receiver IBAN: {payment.receiver_account}"
            )
            result["status"] = "error"
    ...
    def _validate_currency(self, payment: Payment) -> Dict[str, Any]:
        ...
        if payment.country_code == "PL" and payment.currency != "PLN":
            result["warnings"].append(
                f"Non-PLN currency for Polish transaction: {payment.currency}"
            )
            if result["status"] == "valid":
                result["status"] = "warning"
    ...
    def _validate_description(self, payment: Payment) -> Dict[str, Any]:
        ...
        for keyword in suspicious_keywords:
            if keyword in description_lower:
                result["warnings"].append(
                    f"Suspicious keyword in description: {keyword}"
                )
                if result["status"] == "valid":
                    result["status"] = "warning"
    ...
    def _calculate_confidence(
        self, errors: List[str], warnings: List[str], details: Dict[str, Any]
    ) -> float:
...
class ContractorValidator:
    ...
    def _validate_address(self, contractor: Contractor) -> Dict[str, Any]:
        ...
        if not contractor.postal_code:
            ...
            result["warnings"].append(
                f"Invalid postal code format: {contractor.postal_code}"
            )
            if result["status"] == "valid":
                result["status"] = "warning"
...
class AMLMonitor:
        ...
        for keyword in suspicious_keywords:
            if keyword in description_lower:
                alert = AMLAlert(
                    ...
                    detected_at=datetime.now(),
                    details={"keyword": keyword, "description": payment.description},
                    status="new",
                    assigned_to=None,
                )
                alerts.append(alert)
    ...
    def _check_sanctions_list_contractor(
        ...
        for sanction in sanctions_list:
            if sanction.lower() in name_lower:
                alert = AMLAlert(
                    ...
                    detected_at=datetime.now(),
                    details={"sanction": sanction, "contractor_name": contractor.name},
                    status="new",
                    assigned_to=None,
                )
                alerts.append(alert)
        ...
        for pep in pep_list:
            if pep.lower() in name_lower:
                alert = AMLAlert(
                    ...
                    detected_at=datetime.now(),
                    details={"pep": pep, "contractor_name": contractor.name},
                    status="new",
                    assigned_to=None,
                )
                alerts.append(alert)
        ...
        return {
            ...
            "invalid_count": len(
                [
                    r
                    for r in self.validation_results
                    if r.validation_status == ValidationStatus.INVALID
                ]
            ),
            "warning_count": len(
                [
            ...
            "payment_validations": len(
                [r for r in self.validation_results if r.entity_type == "payment"]
            ),
            "contractor_validations": len(
                [r for r in self.validation_results if r.entity_type == "contractor"]
            ),
            "total_aml_alerts": len(self.aml_alerts),
            "critical_aml_alerts": len(
                [a for a in self.aml_alerts if a.risk_level == AMLRiskLevel.CRITICAL]
            ),
            "high_aml_alerts": len(
                [a for a in self.aml_alerts if a.risk_level == AMLRiskLevel.HIGH]
            ),
            "average_confidence": (
                sum(r.confidence for r in self.validation_results)
