...
class RiskAnalyzer:
    ...
    def _generate_recommendations(
        self, risk_factors: List[RiskFactor], overall_level: RiskLevel
    ) -> List[str]:
        ...
        if overall_level == RiskLevel.CRITICAL:
            ...
            recommendations.extend(
                ["Standardowe procedury audytowe", "Podstawowe testy kontroli"]
            )
            ...
            if len(group) > 1:
                for entry in group:
                    anomaly = Anomaly(
                        ...
                        severity=RiskLevel.HIGH,
                        description="Prawdopodobny duplikat zapisu",
                        detected_at=datetime.now(),
                        confidence=0.8,
                        details={
...
class SamplingEngine:
    ...
    def _calculate_mus_sample_size(
        self,
        total_value: float,
        confidence_level: float,
        tolerable_error: float,
        ...
        sample_size = (z_score**2 * total_value * (1 + expected_error)) / (
            tolerable_error * total_value
        )

        return max(int(sample_size), 1)

    def _systematic_sampling(
        self, population: List[Dict[str, Any]], sampling_interval: float
    ) -> List[Dict[str, Any]]:
        ...
        sorted_population = sorted(
            population, key=lambda x: x.get("amount", 0), reverse=True
        )
    ...
    def _calculate_upper_error_limit(
        self,
        selected_items: List[Dict[str, Any]],
        sampling_interval: float,
        confidence_level: float,
        ...
        error_count = sum(
            1 for item in selected_items if item.get("amount", 0) > sampling_interval
        )
...
class AuditAnalytics:
    ...
    def perform_risk_assessment(
        self, custom_risks: List[RiskFactor] = None
    ) -> RiskAssessment:
    ...
    def analyze_journal_entries(self, entries: List[JournalEntry]) -> List[Anomaly]:
        ...
        try:
            ...
            self.logger.info(
                f"Journal entry analysis completed: {len(anomalies)} anomalies found"
            )
            return anomalies
    ...
    def perform_sampling(
        self, population: List[Dict[str, Any]], method: SamplingMethod, **kwargs
    ) -> SamplingResult:
        ...
        try:
            ...
            self.logger.info(
                f"Sampling completed: {method.value}, {result.sample_size} items selected"
            )
            return result
    ...
    def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_risk_anomalies": len(
                [a for a in self.anomalies if a.severity == RiskLevel.HIGH]
            ),
            "critical_risk_anomalies": len(
                [a for a in self.anomalies if a.severity == RiskLevel.CRITICAL]
            ),
            "latest_assessment": (
                self.risk_assessments[-1].id if self.risk_assessments else None
            ),
            "latest_assessment_date": (
                self.risk_assessments[-1].assessment_date.isoformat()
    ...
    def _save_risk_assessment(self, assessment: RiskAssessment):
        ...
        try:
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                json.dump(
                    asdict(assessment), f, indent=2, default=str, ensure_ascii=False
                )
        except Exception as e:
    ...
    def _save_sampling_result(self, result: SamplingResult):
        ...
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            file_path = (
                self.data_dir / f"sampling_{result.method.value}_{timestamp}.json"
            )
            with open(file_path, "w", encoding="utf-8") as f:
