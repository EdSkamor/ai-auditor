                ...
                if company_data.get("registration_date"):
                    try:
                        registration_date = datetime.fromisoformat(
                            company_data["registration_date"].replace("Z", "+00:00")
                        )
                    except:
                        pass
                ...
                if company_data.get("last_updated"):
                    try:
                        last_updated = datetime.fromisoformat(
                            company_data["last_updated"].replace("Z", "+00:00")
                        )
                    except:
                        pass
        ...
        try:
            ...
            for company in result.companies:
                company_data = {
                    ...
                    "last_updated": (
                        company.last_updated.isoformat()
...
class KRSIntegration:
    ...
    def __init__(
        ...
        if HAS_REQUESTS:
            ...
            self.session = None
            self.logger.warning(
                "Requests library not available - KRS integration will use mock data"
            )
    ...
    def _parse_company_data(self, data: Dict[str, Any]) -> CompanyInfo:
        ...
        try:
            ...
            if data.get("registration_date"):
                try:
                    registration_date = datetime.fromisoformat(
                        data["registration_date"].replace("Z", "+00:00")
                    )
                except:
                    pass
    ...
    def search_by_nip(self, nip: str, include_inactive: bool = False) -> KRSResult:
        ...
        try:
            params = {"nip": nip, "include_inactive": include_inactive}
    ...
    def search_by_regon(self, regon: str, include_inactive: bool = False) -> KRSResult:
        ...
        try:
            params = {"regon": regon, "include_inactive": include_inactive}
    ...
    def search_by_name(
        self, name: str, exact_match: bool = True, include_inactive: bool = False
    ) -> KRSResult:
...
def search_company_by_name(
    name: str, exact_match: bool = True, include_inactive: bool = False
) -> KRSResult:
