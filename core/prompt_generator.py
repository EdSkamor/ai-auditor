...
class PromptGenerator:
    ...
    def fill_template(self, template_name: str, **substitutions) -> str:
        ...
        if not prompt_template:
            raise ConfigurationError(
                f"Template '{template_name}' has no prompt_template"
            )
        ...
        if unfilled:
            logger.warning(
                f"Unfilled placeholders in template '{template_name}': {unfilled}"
            )
    ...
    def generate_prompt(self, template_file: str, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if not prompt_template:
                raise ConfigurationError(
                    f"Template file has no prompt_template: {template_file}"
                )
            ...
            raise FileProcessingError(
                f"Failed to generate prompt from {template_file}: {e}"
            )
