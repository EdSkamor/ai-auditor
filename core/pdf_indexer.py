...
class PDFIndexer:
    ...
    def _validate_file_security(self, file_path: Path) -> None:
        ...
        if file_size_mb > self.max_file_size_mb:
            raise FileProcessingError(f"File too large: {file_size_mb:.1f}MB > {self.max_file_size_mb}MB")
    
    def _extract_text_safely(self, file_path: Path) -> str:
        """Extract text from PDF with error handling."""
        if not HAS_PDF_LIBS:
            raise FileProcessingError("PDF libraries not available. Install: pip install pdfplumber pypdf")
            ...
            with pdfplumber.open(file_path) as pdf:
                ...
                for i, page in enumerate(pdf.pages[:pages_to_process]):
                    try:
                        ...
                        self.logger.warning(f"Error extracting page {i+1} from {file_path}: {e}")
                        continue
    ...
    def index_directory(self, directory: Path, recursive: bool = True) -> List[InvoiceData]:
    ...
    def index_zip(self, zip_path: Path, extract_to: Optional[Path] = None) -> List[InvoiceData]:
    ...
    def save_to_csv(self, results: List[InvoiceData], output_path: Path) -> None:
        ...
        for result in results:
            data.append({
                ...
                'issue_date': result.issue_date.strftime('%Y-%m-%d') if result.issue_date else None,
