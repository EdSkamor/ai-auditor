...
class DocumentClassifier:
    ...
    def classify(self, text: str, filename: str = "") -> DocumentClassification:
        ...
        for doc_type, patterns in self.patterns.items():
            ...
            if doc_type == DocumentType.INVOICE and any(word in filename_lower for word in ['faktura', 'invoice']):
                score += 2
            elif doc_type == DocumentType.RECEIPT and any(word in filename_lower for word in ['paragon', 'receipt']):
                score += 2
            elif doc_type == DocumentType.CONTRACT and any(word in filename_lower for word in ['umowa', 'contract']):
                score += 2
            elif doc_type == DocumentType.BANK_STATEMENT and any(word in filename_lower for word in ['wyciąg', 'statement']):
                score += 2
            elif doc_type == DocumentType.TAX_DOCUMENT and any(word in filename_lower for word in ['jpk', 'deklaracja', 'vat']):
                score += 2
...
class FieldExtractor:
    ...
    def extract_fields(self, text: str, document_type: DocumentType, ocr_result: OCRResult) -> List[ExtractedField]:
    ...
    def _extract_invoice_fields(self, text: str, ocr_result: OCRResult) -> List[ExtractedField]:
        """Ekstrakcja pól faktury."""
        fields = []
        
        # Invoice number
        invoice_number = self._extract_pattern(text, r'(?:nr|numer|number)\s*:?\s*([a-zA-Z0-9\-/]+)', 'invoice_number')
        if invoice_number:
        ...
        # Date
        date = self._extract_pattern(text, r'(?:data|date)\s*:?\s*(\d{1,2}[./-]\d{1,2}[./-]\d{2,4})', 'date')
        if date:
            fields.append(ExtractedField(
                field_name='date',
                value=date,
        ...
        # Seller
        seller = self._extract_pattern(text, r'(?:sprzedawca|seller)\s*:?\s*([^\n]+)', 'seller')
        if seller:
            fields.append(ExtractedField(
                field_name='seller',
                value=seller,
        ...
        # Buyer
        buyer = self._extract_pattern(text, r'(?:nabywca|buyer)\s*:?\s*([^\n]+)', 'buyer')
        if buyer:
            fields.append(ExtractedField(
                field_name='buyer',
                value=buyer,
        ...
        # Net amount
        net_amount = self._extract_pattern(text, r'(?:netto|net)\s*:?\s*([0-9\s,.-]+)', 'net_amount')
        if net_amount:
            fields.append(ExtractedField(
        ...
        # VAT amount
        vat_amount = self._extract_pattern(text, r'(?:vat|podatek)\s*\d*%?\s*:?\s*([0-9\s,.-]+)', 'vat_amount')
        if vat_amount:
            fields.append(ExtractedField(
        ...
        # Gross amount
        gross_amount = self._extract_pattern(text, r'(?:brutto|gross|total)\s*:?\s*([0-9\s,.-]+)', 'gross_amount')
        if gross_amount:
            fields.append(ExtractedField(
    ...
    def _extract_receipt_fields(self, text: str, ocr_result: OCRResult) -> List[ExtractedField]:
        """Ekstrakcja pól paragonu."""
        fields = []
        
        # Receipt number
        receipt_number = self._extract_pattern(text, r'(?:nr|numer)\s*:?\s*([0-9]+)', 'receipt_number')
        if receipt_number:
            fields.append(ExtractedField(
        ...
        # Total amount
        total_amount = self._extract_pattern(text, r'(?:suma|total|kwota)\s*:?\s*([0-9\s,.-]+)', 'total_amount')
        if total_amount:
            fields.append(ExtractedField(
    ...
    def _extract_contract_fields(self, text: str, ocr_result: OCRResult) -> List[ExtractedField]:
        """Ekstrakcja pól umowy."""
        fields = []
        
        # Contract number
        contract_number = self._extract_pattern(text, r'(?:nr|numer)\s*umowy\s*:?\s*([a-zA-Z0-9\-/]+)', 'contract_number')
        if contract_number:
            fields.append(ExtractedField(
        ...
        # Contract date
        date = self._extract_pattern(text, r'(?:zawarta\s*w\s*dniu|data)\s*:?\s*(\d{1,2}[./-]\d{1,2}[./-]\d{2,4})', 'date')
        if date:
            fields.append(ExtractedField(
                field_name='date',
                value=date,
    ...
    def _extract_bank_statement_fields(self, text: str, ocr_result: OCRResult) -> List[ExtractedField]:
        """Ekstrakcja pól wyciągu bankowego."""
        fields = []
        
        # Account number
        account_number = self._extract_pattern(text, r'(?:rachunek|account)\s*:?\s*([0-9\s-]+)', 'account_number')
        if account_number:
            fields.append(ExtractedField(
        ...
        # Period
        period = self._extract_pattern(text, r'(?:okres|period)\s*:?\s*(\d{1,2}[./-]\d{1,2}[./-]\d{2,4}\s*-\s*\d{1,2}[./-]\d{1,2}[./-]\d{2,4})', 'period')
        if period:
            fields.append(ExtractedField(
                field_name='period',
                value=period,
    ...
    def _extract_tax_document_fields(self, text: str, ocr_result: OCRResult) -> List[ExtractedField]:
        """Ekstrakcja pól dokumentu podatkowego."""
        fields = []
        
        # Tax period
        period = self._extract_pattern(text, r'(?:okres|period)\s*:?\s*(\d{1,2}[./-]\d{1,2}[./-]\d{2,4})', 'period')
        if period:
            fields.append(ExtractedField(
                field_name='period',
                value=period,
        ...
        # Tax amount
        tax_amount = self._extract_pattern(text, r'(?:podatek|tax)\s*:?\s*([0-9\s,.-]+)', 'tax_amount')
        if tax_amount:
            fields.append(ExtractedField(
    ...
    def _extract_generic_fields(self, text: str, ocr_result: OCRResult) -> List[ExtractedField]:
...
class ETLProcessor:
    ...
    def process_directory(self, directory: Path, recursive: bool = True) -> List[ETLResult]:
    ...
    def _save_result(self, result: ETLResult):
        ...
        try:
            ...
            with open(fields_file, 'w', encoding='utf-8') as f:
                json.dump([asdict(field) for field in result.extracted_fields], f, indent=2, ensure_ascii=False)
        ...
        try:
            ...
            if format == "json":
                ...
                with open(output_file, 'w', encoding='utf-8') as f:
                    json.dump([asdict(result) for result in self.results], f, indent=2, default=str, ensure_ascii=False)
                ...
                for result in self.results:
                    ...
                    for field in result.extracted_fields:
                        ...
                        base_data[f'field_{field.field_name}_confidence'] = field.confidence
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        # Average confidences
        avg_classification_confidence = sum(r.classification_confidence for r in self.results) / len(self.results)
        avg_ocr_confidence = sum(r.ocr_result.confidence for r in self.results) / len(self.results)
        ...
        return {
            ...
            'total_processing_time': round(sum(r.processing_time for r in self.results), 3)
        }
