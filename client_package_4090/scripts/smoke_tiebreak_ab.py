...
def create_tiebreaker_test_data():
    ...
    pop_data = pd.DataFrame({
        ...
        'Kontrahent': ['ACME Corporation', 'ACME Corp', 'Test Company', 'Test Co Ltd']
    })
...
def test_tiebreaker_v1():
    ...
    try:
        ...
        matcher_v1 = POPMatcher(
            tiebreak_weight_fname=0.8,
            tiebreak_min_seller=0.3,
...
def test_deterministic_behavior():
    ...
    try:
        ...
        all_identical = all(
            result['status'] == first_result['status'] and
            result['criteria'] == first_result['criteria'] and
            result['pop_row_index'] == first_result['pop_row_index']
...
def test_confidence_scoring():
    ...
    try:
        ...
        if result.confidence > 0.8:
            ...
            print(f"⚠️  Perfect match confidence lower than expected: {result.confidence}")
...
def compare_ab_results():
    ...
    try:
        ...
        if v1_result and v2_result:
            print("\n📊 A/B Test Comparison:")
            print(f"V1 (High filename weight):")
            ...
            print(f"V2 (Low filename weight, high seller threshold):")
            ...
            if (v1_result.filename_hit != v2_result.filename_hit or 
                v1_result.criteria != v2_result.criteria):
