...
class ValidateCLI(BaseCLI):
    ...
    def _setup_validation_args(self) -> None:
        ...
        mode_group.add_argument(
            "--demo",
            action="store_true",
            help="Demo mode: validate single file"
        )
        mode_group.add_argument(
            "--bulk",
            action="store_true", 
            help="Bulk mode: validate multiple files"
        )
        ...
        self.parser.add_argument(
            "--file", "-f",
            type=Path,
            help="Input file for demo mode (PDF or Excel)"
        )
        self.parser.add_argument(
            "--input-dir", "-i",
            type=Path,
            help="Input directory for bulk mode"
        )
        self.parser.add_argument(
            "--pop-file",
            type=Path,
            help="POP data file (Excel/CSV)"
        )
        ...
        self.parser.add_argument(
            "--enable-ocr",
            action="store_true",
            help="Enable OCR for PDF processing"
        )
        self.parser.add_argument(
            "--max-file-size-mb",
            type=int,
    ...
    def _validate_args(self, args: argparse.Namespace) -> None:
        ...
        if args.demo:
            if not args.file:
                raise CLIError("--file is required for demo mode", ExitCode.INVALID_ARGS)
            if not args.file.exists():
                raise CLIError(f"Input file not found: {args.file}", ExitCode.FILE_NOT_FOUND)
        
        elif args.bulk:
            if not args.input_dir:
                raise CLIError("--input-dir is required for bulk mode", ExitCode.INVALID_ARGS)
            if not args.input_dir.exists():
                raise CLIError(f"Input directory not found: {args.input_dir}", ExitCode.FILE_NOT_FOUND)
        
        if args.pop_file and not args.pop_file.exists():
            raise CLIError(f"POP file not found: {args.pop_file}", ExitCode.FILE_NOT_FOUND)
    ...
    def _run_demo_validation(self, args: argparse.Namespace, config: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if pop_data is not None:
                ...
                match_result = match_invoice(invoice_data, pop_data, pdf_result.source_filename)
    ...
    def _run_bulk_validation(self, args: argparse.Namespace, config: Dict[str, Any]) -> int:
        ...
        try:
            ...
            self.logger.info(f"Audit completed successfully")
