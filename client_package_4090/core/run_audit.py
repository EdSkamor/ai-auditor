...
from .pdf_indexer import PDFIndexer, InvoiceData
from .pop_matcher import POPMatcher, MatchResult
...
class AuditPipeline:
    ...
    def _index_pdfs(self, pdf_source: Path) -> None:
        ...
        try:
            if pdf_source.is_file() and pdf_source.suffix.lower() == '.zip':
                ...
                self.pdf_results = self.pdf_indexer.index_directory(pdf_source, recursive=True)
            else:
    ...
    def _load_pop_data(self, pop_file: Path) -> None:
        ...
        try:
            ...
            self.logger.info(f"Loaded POP data: {self.pop_data.shape[0]} rows, {self.pop_data.shape[1]} columns")
            
            # Validate required columns
            required_columns = ['numer', 'data', 'netto', 'kontrahent']
            missing_columns = []
            for col in required_columns:
                if not any(col.lower() in str(c).lower() for c in self.pop_data.columns):
    ...
    def _match_invoices(self) -> None:
        ...
        try:
            ...
            for i, pdf_result in enumerate(self.pdf_results, 1):
                self.logger.info(f"Matching invoice {i}/{len(self.pdf_results)}: {pdf_result.source_filename}")
                ...
                match_result = self.pop_matcher.match_invoice(
                    invoice_data, 
                    self.pop_data, 
                    pdf_result.source_filename
                )
                
                self.match_results.append(match_result)
            
            self.logger.info(f"Completed matching for {len(self.match_results)} invoices")
    ...
    def _generate_reports(self, run_name: str) -> Dict[str, Path]:
        ...
        try:
            ...
            verdicts_data = []
            for i, (pdf_result, match_result) in enumerate(zip(self.pdf_results, self.match_results)):
                verdict_data = {
                    'source_filename': pdf_result.source_filename,
                    'source_path': pdf_result.source_path,
                    'invoice_id': pdf_result.invoice_id,
                    'issue_date': pdf_result.issue_date.strftime('%Y-%m-%d') if pdf_result.issue_date else None,
                    'total_net': pdf_result.total_net,
                    'currency': pdf_result.currency,
                    'seller_guess': pdf_result.seller_guess,
            ...
            summary_stats = {
                ...
                'matched_invoices': len(verdicts_df[verdicts_df['status'] == 'znaleziono']),
                'unmatched_invoices': len(verdicts_df[verdicts_df['status'] == 'brak']),
                'error_invoices': len(verdicts_df[verdicts_df['status'] == 'error']),
                'match_rate': len(verdicts_df[verdicts_df['status'] == 'znaleziono']) / len(verdicts_df) if len(verdicts_df) > 0 else 0,
    ...
    def _create_summary(self, start_time: datetime, report_paths: Dict[str, Path]) -> Dict[str, Any]:
        """Create final summary."""
        end_time = datetime.now()
        duration = (end_time - start_time).total_seconds()
        
        summary = {
            'run_name': report_paths.get('excel_report', Path()).stem.replace('Audyt_koncowy_', ''),
            'start_time': start_time.isoformat(),
            'end_time': end_time.isoformat(),
            'duration_seconds': duration,
            'total_pdfs_processed': len(self.pdf_results),
            'total_matches': len([r for r in self.match_results if r.status.value == 'znaleziono']),
            'total_unmatched': len([r for r in self.match_results if r.status.value == 'brak']),
            'total_errors': len([r for r in self.match_results if r.status.value == 'error']),
