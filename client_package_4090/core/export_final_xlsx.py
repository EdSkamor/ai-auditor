...
class ExcelReportGenerator:
    ...
    def _apply_table_style(self, ws, start_row: int, end_row: int, start_col: int, end_col: int):
        ...
        for row in range(start_row, end_row + 1):
            for col in range(start_col, end_col + 1):
                ...
                if row % 2 == 0:
                    cell.fill = PatternFill(start_color=self.colors['light_blue'],
    ...
    def _create_detailed_results_sheet(self, wb: Workbook, results_df: pd.DataFrame) -> None:
        """Create detailed results sheet."""
        ws = wb.create_sheet("Detailed Results")
        
        row = 1
        row = self._apply_header_style(ws, row, len(results_df.columns), "Detailed Invoice Matching Results")
        row += 1
        ...
        for _, data_row in results_df.iterrows():
            for col, value in enumerate(data_row, 1):
                ...
                if col == 1 and str(value).lower() == 'znaleziono':
                    ...
                    cell.fill = PatternFill(start_color=self.colors['light_red'],
    ...
    def _create_top50_mismatches_sheet(self, wb: Workbook, mismatches_df: pd.DataFrame) -> None:
        """Create Top 50 mismatches sheet."""
        ws = wb.create_sheet("Top 50 Mismatches")
        
        row = 1
        row = self._apply_header_style(ws, row, len(mismatches_df.columns), "Top 50 Invoice Mismatches")
        row += 1
    ...
    def _create_statistics_sheet(self, wb: Workbook, stats_data: Dict[str, Any]) -> None:
...
def generate_excel_report(all_invoices_df: pd.DataFrame,
                         verdicts_df: pd.DataFrame,
                         summary_data: Dict[str, Any],
                         output_path: Path) -> None:
    """Convenience function to generate Excel report."""
    _excel_generator.generate_report(all_invoices_df, verdicts_df, summary_data, output_path)
