...
# --- Ustawienia (produkcyjnie ustaw przez systemd/ENV) ---
ALLOW_ORIGINS = [
    ...
    try:
        ...
        if amount_col is None:
            candidates = [
                ...
                if any(
                    k in c
                    for k in ["kwota", "wartosc", "amount", "netto", "brutto", "vat"]
                )
            ]
        ...
        if amount_col and amount_col in df.columns:
            ...
            metrics["amount_column"] = amount_col
            metrics["amount_sum"] = float(ser.sum(skipna=True))
            metrics["amount_mean"] = float(ser.mean(skipna=True))

        # Próbka tabeli (pierwsze 10 wierszy)
        sample = df.head(10).fillna("").astype(str).to_dict(orient="records")

        # (opcjonalnie) zapis uploadu
        saved_to = None
        if SAVE_UPLOADS:
            with NamedTemporaryFile(
                prefix="upload_", suffix=f"-{file.filename}", delete=False
            ) as tmp:
                tmp.write(content)
                saved_to = tmp.name
