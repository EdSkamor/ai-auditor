...
class GenerateRiskTableCLI(BaseCLI):
    ...
    def _setup_risk_table_args(self) -> None:
        ...
        self.parser.add_argument(
            "--formulas",
            type=Path,
            help="Risk formulas configuration file (JSON)"
        )
        self.parser.add_argument(
            "--risk-definitions",
            type=Path,
            help="Risk definitions file (JSON)"
        )
    ...
    def _validate_args(self, args: argparse.Namespace) -> None:
        ...
        if args.formulas and not args.formulas.exists():
            raise CLIError(f"Formulas file not found: {args.formulas}", ExitCode.FILE_NOT_FOUND)
        
        if args.risk_definitions and not args.risk_definitions.exists():
            raise CLIError(f"Risk definitions file not found: {args.risk_definitions}", ExitCode.FILE_NOT_FOUND)
        
        if args.template and not args.template.exists():
            raise CLIError(f"Template file not found: {args.template}", ExitCode.FILE_NOT_FOUND)
        
        if not (0.0 <= args.confidence_level <= 1.0):
            raise CLIError("Confidence level must be between 0.0 and 1.0", ExitCode.INVALID_ARGS)
    ...
    def _run_impl(self, args: argparse.Namespace, config: Dict[str, Any]) -> int:
        ...
        try:
            ...
            # Import risk table generator
            from core.risk_table_generator import RiskTableGenerator, generate_sample_risk_assessment
            ...
            if args.sample:
                ...
                self.logger.info("Generating sample risk assessment...")
                
                output_path = args.output_dir / f"risk_assessment_sample_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                ...
                self.logger.info(f"Sample risk assessment generated: {result_path}")
                
            else:
                # Generate custom risk assessment
                self.logger.info("Generating custom risk assessment...")
                
                # Parse risk items from input file if provided
                risk_items = []
                if args.input_file and args.input_file.exists():
                    self.logger.info(f"Loading risk items from {args.input_file}")
                    
                    # Load risk items from JSON file
                    import json
                    with open(args.input_file, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                    
                    for item_data in data.get('risk_items', []):
                        from core.risk_table_generator import RiskItem
                ...
                output_path = args.output_dir / f"risk_assessment_{args.company_name.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                
                result_path = generator.generate_excel_workbook(assessment, output_path)
                
                self.logger.info(f"Risk assessment generated: {result_path}")
                
                # Print summary
                self.logger.info(f"Risk Assessment Summary:")
                self.logger.info(f"  Company: {assessment.company_name}")
                self.logger.info(f"  Total risks: {assessment.total_risks}")
                self.logger.info(f"  High risks: {assessment.high_risks}")
                self.logger.info(f"  Medium risks: {assessment.medium_risks}")
                self.logger.info(f"  Low risks: {assessment.low_risks}")
                self.logger.info(f"  Overall score: {assessment.overall_score:.2f}")
