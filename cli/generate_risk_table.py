    ...
    def _run_impl(self, args: argparse.Namespace, config: Dict[str, Any]) -> int:
        ...
        try:
            ...
            from core.risk_table_generator import (
                RiskTableGenerator,
                generate_sample_risk_assessment,
            )
            ...
            if args.sample:
                ...
                output_path = (
                    args.output_dir
                    / f"risk_assessment_sample_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                )

                result_path = generate_sample_risk_assessment(
                    company_name=args.company_name, output_path=output_path
                )
                ...
                output_path = (
                    args.output_dir
                    / f"risk_assessment_{args.company_name.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                )
                ...
                # Print summary
                self.logger.info("Risk Assessment Summary:")
