...
class EnrichDataCLI(BaseCLI):
    ...
    def _setup_enrichment_args(self) -> None:
        ...
        self.parser.add_argument(
            "--krs",
            action="store_true",
            help="Enable KRS data enrichment"
        )
        self.parser.add_argument(
            "--whitelist",
            action="store_true",
            help="Enable whitelist data enrichment"
        )
        self.parser.add_argument(
            "--krs-api-key",
        ...
        self.parser.add_argument(
            "--whitelist-file",
            type=Path,
            help="Whitelist data file (CSV/Excel)"
        )
    ...
    def _validate_args(self, args: argparse.Namespace) -> None:
        ...
        if not args.input.exists():
            raise CLIError(f"Input file not found: {args.input}", ExitCode.FILE_NOT_FOUND)
        
        if not (args.krs or args.whitelist):
            raise CLIError("At least one enrichment source (--krs or --whitelist) must be specified", ExitCode.INVALID_ARGS)
        
        if args.krs and not args.krs_api_key:
            import os
            if not os.getenv("KRS_API_KEY"):
                raise CLIError("KRS API key required (--krs-api-key or KRS_API_KEY environment variable)", ExitCode.INVALID_ARGS)
        
        if args.whitelist and args.whitelist_file and not args.whitelist_file.exists():
            raise CLIError(f"Whitelist file not found: {args.whitelist_file}", ExitCode.FILE_NOT_FOUND)
    ...
    def _run_impl(self, args: argparse.Namespace, config: Dict[str, Any]) -> int:
        ...
        try:
            ...
            from core.krs_integration import KRSIntegration, search_company_by_nip, search_company_by_regon, search_company_by_name
            ...
            if args.input_file and args.input_file.exists():
                ...
                if args.output_format == 'csv':
                    output_path = args.output_dir / f"enriched_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                    enriched_df.to_csv(output_path, index=False, encoding='utf-8')
                else:
                    output_path = args.output_dir / f"enriched_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                    enriched_df.to_excel(output_path, index=False)
                
                self.logger.info(f"Enriched data saved to: {output_path}")
                
                # Print summary
                enriched_count = sum(1 for c in enriched_companies if c.get('krs_enriched', False))
                self.logger.info(f"Enrichment summary:")
                self.logger.info(f"  Total companies: {len(enriched_companies)}")
                self.logger.info(f"  Successfully enriched: {enriched_count}")
                self.logger.info(f"  Failed: {len(enriched_companies) - enriched_count}")
                ...
                if args.nip:
                    ...
                    result = search_company_by_name(args.company_name, args.exact_match, args.include_inactive)
                else:
                    raise ValueError("No search criteria provided (NIP, REGON, or company name)")
                ...
                # Print results
                self.logger.info(f"Found {result.total_found} companies (query time: {result.query_time:.2f}s)")
                ...
                for i, company in enumerate(result.companies, 1):
                    ...
                    if company.registration_date:
                        self.logger.info(f"  Registration date: {company.registration_date.strftime('%Y-%m-%d')}")
                
                # Save results
                if args.save_results:
                    import json
                    output_path = args.output_dir / f"krs_search_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
