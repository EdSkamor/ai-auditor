...
class OCRSampleCLI(BaseCLI):
    ...
    def _setup_ocr_args(self) -> None:
        ...
        self.parser.add_argument(
            "--gpu-enabled", action="store_true", help="Enable GPU acceleration for OCR"
        )
        self.parser.add_argument(
    ...
    def _run_impl(self, args: argparse.Namespace, config: Dict[str, Any]) -> int:
        ...
        try:
            ...
            ocr_processor = OCRProcessor(
                engine=args.engine, language=args.language, gpu_enabled=args.gpu_enabled
            )
            ...
            for i, file_path in enumerate(files_to_process[: args.sample_size], 1):
                self.logger.info(
                    f"Processing {i}/{min(len(files_to_process), args.sample_size)}: {file_path.name}"
                )
                ...
                try:
                    ...
                    if args.verbose:
                        ...
                        if invoice_fields.invoice_number:
                            self.logger.info(
                                f"  Invoice number: {invoice_fields.invoice_number}"
                            )
            ...
            if results:
                ...
                self.logger.info(
                    f"OCR sampling completed: {successful}/{len(results)} files processed successfully"
                )
